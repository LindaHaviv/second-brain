...
stored in Oracle (wiki_pages + page_sources + page_links). Re-run as content grows -> it
recompiles, so the knowledge layer self-improves.
"""
import json
...
def compile_page(client, conn, topic, all_topics):
    ...
    valid = {h["post_id"] for h in hits}
    src = "\n".join(f"[{h['post_id']}] ({h['platform_id']}/{h['kind']}) {h['title']}: "
                    f"{(h['snippet'] or '')[:160]}" for h in hits)
...
def build_wiki(client, conn, n=10):
    ...
    # resolve cross-links (topic name -> page_id)
